...
CREATE OR REPLACE PROCEDURE "SP_MEDIAN_RANGE_CHECK"("RULE" VARIANT)
RETURNS NUMBER(38,0)
LANGUAGE SQL
EXECUTE AS CALLER
AS '
DECLARE
    ...
    -- Median check specific variables
    v_min_value FLOAT;
    v_max_value FLOAT;
...
BEGIN
    ...
    v_procedure_name := COALESCE(RULE:PROCEDURE_NAME::STRING, ''SP_MEDIAN_RANGE_CHECK'');
    ...
    IF (v_error_message IS NULL) THEN
        v_sql := ''SELECT COUNT(*) AS total_count, COUNT_IF("'' || v_column_nm || ''" IS NULL) as missing_count, MEDIAN('' || v_column_expr || ''::FLOAT) AS observed_median FROM '' ||
                 v_from_clause ;
        ...
        BEGIN
            ...
            FOR record IN v_cursor DO
                ...
                v_observed_value := record.observed_median;
                BREAK;
            END FOR;
    ...
    v_log_message := ''Not applicable. Failed row/key capture is not performed for aggregate checks like MEDIAN.'';
    ...
    IF (v_error_message IS NULL) THEN
        ...
        LET results_json_str STRING := ''{'' ||
            ''"observed_value": '' || COALESCE(v_observed_value::FLOAT::STRING, ''null'') || '','' ||
        ...
        v_sql := ''INSERT INTO "'' || v_dq_db_name || ''"."'' || v_dq_schema_name || ''".DQ_RULE_RESULTS (
                ...
                COALESCE(v_observed_value::FLOAT::STRING, ''NULL'') || ''''''::VARIANT, NULL::VARIANT, NULL::VARIANT, '' || 
...
EXCEPTION
    WHEN OTHER THEN
        BEGIN
            ...
            VALUES (COALESCE(:v_run_id, -1), COALESCE(:v_check_config_id, -1), COALESCE(:v_procedure_name, ''SP_MEDIAN_RANGE_CHECK''), COALESCE(:v_step, ''UNKNOWN''), CURRENT_TIMESTAMP(), CURRENT_TIMESTAMP(), ''FAILED'', :v_error_message);
