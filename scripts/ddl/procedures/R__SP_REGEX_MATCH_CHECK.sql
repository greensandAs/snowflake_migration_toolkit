...
BEGIN
    ...
    -- Pattern mismatch condition: value is not null and does not match regex
    v_where_clause_condition := ''"'' || v_column_nm || ''" IS NOT NULL AND NOT REGEXP_LIKE(CAST("'' || v_column_nm || ''" AS VARCHAR), '''''' || v_regex_pattern || '''''')'';
