...
CREATE OR REPLACE PROCEDURE "SP_REGEX_MATCH_CHECK"("RULE" VARIANT)
RETURNS NUMBER(38, 0)
LANGUAGE SQL
EXECUTE AS CALLER
AS '
DECLARE
    ...
    -- Regex check specific variables
    v_regex_pattern STRING;
...
BEGIN
    ...
    v_procedure_name := COALESCE(RULE:PROCEDURE_NAME::STRING, ''SP_REGEX_MATCH_CHECK'');
    ...
    BEGIN
        ...
        v_regex_pattern := v_kwargs_variant:regex::STRING;
        ...
        IF (v_column_nm IS NULL OR v_regex_pattern IS NULL) THEN
             v_error_message := ''Required rule parameter (COLUMN_NAME or REGEX pattern) is missing.'';             
    ...
    -- Pattern mismatch condition: value is null/blank or does not match regex (TRIM before checking)
    v_where_clause_condition := ''"'' || v_column_nm || ''" IS NULL OR TRIM(CAST("'' || v_column_nm || ''" AS VARCHAR)) = '''''''' OR NOT REGEXP_LIKE(TRIM(CAST("'' || v_column_nm || ''" AS VARCHAR)), '''''' || v_regex_pattern || '''''')'';
    ...
    BEGIN
        ...
        -- Framework standard: v_percent is primary success/fail metric
    ...
    IF (v_unexpected > 0) THEN
        BEGIN
            ...
            EXECUTE IMMEDIATE v_sql;
            v_log_message := SQLROWCOUNT || '' rows inserted into structured failure table.'';
        EXCEPTION
            WHEN OTHER THEN
    ...
    ----------------------------------------------------------------------------------------------------
    -- 7. Insert Results into DQ_RULE_RESULTS Table
    ...
    BEGIN
        LET details_json_str STRING := ''{"regex_pattern": "'' || REPLACE(v_regex_pattern, '''''''', '''''''''''') || ''"}'';
...
EXCEPTION
    WHEN OTHER THEN
        ...
        VALUES (COALESCE(:v_run_id, -1), COALESCE(:v_check_config_id, -1), COALESCE(:v_procedure_name, ''SP_REGEX_MATCH_CHECK''), COALESCE(:v_step, ''UNKNOWN''), CURRENT_TIMESTAMP(), CURRENT_TIMESTAMP(), ''FAILED'', :v_error_message);
