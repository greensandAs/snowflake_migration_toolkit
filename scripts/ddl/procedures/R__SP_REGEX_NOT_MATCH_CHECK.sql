...
CREATE OR REPLACE PROCEDURE "SP_REGEX_NOT_MATCH_CHECK"("RULE" VARIANT)
RETURNS NUMBER(38, 0)
LANGUAGE SQL
EXECUTE AS CALLER
AS '
...
BEGIN
    ...
    v_procedure_name := COALESCE(RULE:PROCEDURE_NAME::STRING, ''SP_REGEX_NOT_MATCH_CHECK'');
    ...
    -- Pattern mismatch condition for NOT_MATCH: rows are "unexpected" IF they match the regex (treat blank as NULL)
    v_where_clause_condition := ''"'' || v_column_nm || ''" IS NOT NULL AND TRIM(CAST("'' || v_column_nm || ''" AS VARCHAR)) <> '''''''' AND REGEXP_LIKE(TRIM(CAST("'' || v_column_nm || ''" AS VARCHAR)), '''''' || v_regex_pattern || '''''')'';
