...
BEGIN
    ...
    -- Pattern mismatch condition for NOT_MATCH: rows are "unexpected" IF they match the regex
    v_where_clause_condition := ''"'' || v_column_nm || ''" IS NOT NULL AND REGEXP_LIKE(CAST("'' || v_column_nm || ''" AS VARCHAR), '''''' || v_regex_pattern || '''''')'';
