...
CREATE OR REPLACE PROCEDURE "SP_TABLE_COLUMN_COUNT_EQUAL_CHECK"("RULE" VARIANT)
RETURNS NUMBER(38,0)
LANGUAGE SQL
EXECUTE AS CALLER
AS '
DECLARE
    ...
    -- Column count check specific variables
    v_expected_value FLOAT; -- Expected column count
    ...
    v_temp_view_name STRING; -- New variable for temporary view
    ...
    -- Variables used to map equality to result JSON structure
    v_min_value FLOAT;
    v_max_value FLOAT;
...
BEGIN
    ...
    v_procedure_name := COALESCE(RULE:PROCEDURE_NAME::STRING, ''SP_TABLE_COLUMN_COUNT_EQUAL_CHECK'');
    ...
    BEGIN
        ...
        v_min_value := v_expected_value;
        v_max_value := v_expected_value;
        v_strict_min := FALSE;
        v_strict_max := FALSE;

        -- Validation
        IF (v_expected_value IS NULL) THEN
            v_error_message := ''KWARGS must contain "value" for the expected column count.'';
            v_status_code := v_execution_error;
            UPDATE DQ_RULE_AUDIT_LOG SET END_TIMESTAMP = CURRENT_TIMESTAMP(), STATUS = ''FAILED'', ERROR_MESSAGE = :v_error_message, LOG_MESSAGE = :v_input_rule_str WHERE DATASET_RUN_ID = :v_run_id AND RULE_CONFIG_ID = :v_check_config_id AND STEP_NAME = :v_step;
            RETURN v_status_code;
    ...
    IF (v_error_message IS NULL) THEN
        ...
        -- Logic for QUERY type: Create a temporary view and query its metadata
        IF (UPPER(v_dataset_type) = ''QUERY'') THEN
            v_temp_view_name := ''DQ_COL_COUNT_TEMP_'' || REPLACE(TO_VARCHAR(UUID_STRING()), ''-'', ''_'');
            
            -- Set target database/schema for temporary view creation
            v_from_clause := ''"'' || v_database_name || ''"."'' || v_schema_name || ''"'';

            -- 3a. Create temporary view (with a LIMIT 0 to avoid scanning all data)
            BEGIN
                ...
                -- NOTE: Creating a TEMPORARY VIEW over a query automatically stores the metadata. 
                ...
                v_sql := ''SELECT COUNT(*) AS observed_column_count FROM "'' || v_database_name || ''COLUMNS WHERE TABLE_SCHEMA = '''''' || v_schema_name || '''''' AND TABLE_NAME = '''''' || v_temp_view_name || '''''''';
            EXCEPTION
                WHEN OTHER THEN
        ...
        -- Logic for TABLE type: Query the existing table metadata
        ELSE -- v_dataset_type = ''TABLE''
            v_log_message := ''Querying INFORMATION_SCHEMA for permanent table column count.'';
            v_sql := ''SELECT COUNT(*) AS observed_column_count FROM "'' || v_database_name || ''COLUMNS WHERE TABLE_SCHEMA = '''''' || v_schema_name || '''''' AND TABLE_NAME = '''''' || v_table_name || '''''''';
        END IF;
        ...
        BEGIN
            ...
            IF (v_observed_column_count != v_expected_value) THEN
                v_status_code_flag := 1;
                v_error_message := ''Column count failed equality check: Observed ('' || v_observed_column_count || '') is not equal to expected ('' || v_expected_value || '').'';
            END IF;
            ...
            ELSE
                v_log_message := ''Column count validation passed. Observed count ('' || v_observed_column_count || '') equals expected count ('' || v_expected_value || '').'';
            END IF;
...
EXCEPTION
    WHEN OTHER THEN
        BEGIN
            ...
            VALUES (COALESCE(:v_run_id, -1), COALESCE(:v_check_config_id, -1), COALESCE(:v_procedure_name, ''SP_TABLE_COLUMN_COUNT_EQUAL_CHECK''), COALESCE(v_step, ''UNKNOWN''), CURRENT_TIMESTAMP(), CURRENT_TIMESTAMP(), ''FAILED'', :v_error_message);
