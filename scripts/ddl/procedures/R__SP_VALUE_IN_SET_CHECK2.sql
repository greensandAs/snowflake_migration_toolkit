...
CREATE OR REPLACE PROCEDURE "SP_VALUE_IN_SET_CHECK2"("RULE" VARIANT)
RETURNS NUMBER(38, 0)
LANGUAGE SQL
EXECUTE AS CALLER
AS '
DECLARE
    ...
    -- Value In Set specific variables
...
BEGIN
    ...
    v_procedure_name := COALESCE(RULE:PROCEDURE_NAME::STRING, ''SP_VALUE_IN_SET_CHECK'');
    ...
    BEGIN
        ...
        ELSEIF (v_column_nm IS NULL OR v_value_set_list IS NULL OR ARRAY_SIZE(v_value_set_list) = 0) THEN
            v_error_message := ''COLUMN_NAME and non-empty VALUE_SET are required for SP_VALUE_IN_SET_CHECK.'';
            v_status_code := v_execution_error;
            UPDATE DQ_RULE_AUDIT_LOG SET END_TIMESTAMP = CURRENT_TIMESTAMP(), STATUS = ''FAILED'', ERROR_MESSAGE = :v_error_message, LOG_MESSAGE = :v_input_rule_str WHERE DATASET_RUN_ID = :v_run_id AND RULE_CONFIG_ID = :v_check_config_id AND STEP_NAME = :v_step;
            RETURN v_status_code;
        END IF;
...
EXCEPTION
    WHEN OTHER THEN
        ...
        VALUES (COALESCE(:v_run_id, -1), COALESCE(:v_check_config_id, -1), COALESCE(:v_procedure_name, ''SP_VALUE_IN_SET_CHECK''), COALESCE(:v_step, ''UNKNOWN''), CURRENT_TIMESTAMP(), CURRENT_TIMESTAMP(), ''FAILED'', :v_error_message);
