...
CREATE OR REPLACE PROCEDURE "SP_VALUE_LENGTH_EQUAL_CHECK"("RULE" VARIANT)
RETURNS NUMBER(38,0)
LANGUAGE SQL
EXECUTE AS CALLER
AS '
DECLARE
    ...
    -- Value Length check specific variables
    v_expected_value INT; 
...
BEGIN
    ...
    v_procedure_name := COALESCE(RULE:PROCEDURE_NAME::STRING, ''SP_VALUE_LENGTH_EQUAL_CHECK'');
    ...
    BEGIN
        ...
        v_expected_value := v_kwargs_variant:value::INT;
        v_column_nm := COALESCE(v_kwargs_variant:column::STRING, RULE:COLUMN_NM::STRING);
        ...
        IF (v_column_nm IS NULL OR v_expected_value IS NULL) THEN
            v_error_message := ''Required rule parameters (COLUMN_NM or value) are missing.'';
            v_status_code := v_execution_error;
            UPDATE DQ_RULE_AUDIT_LOG SET END_TIMESTAMP = CURRENT_TIMESTAMP(), STATUS = ''FAILED'', ERROR_MESSAGE = :v_error_message, LOG_MESSAGE = :v_input_rule_str WHERE DATASET_RUN_ID = :v_run_id AND RULE_CONFIG_ID = :v_check_config_id AND STEP_NAME = :v_step;
            RETURN v_status_code;
    ...
    v_where_clause_condition := ''"'' || v_column_nm || ''" IS NOT NULL AND LENGTH("'' || v_column_nm || ''") != '' || v_expected_value;
    ...
    IF (v_unexpected > 0) THEN
        ...
        EXCEPTION
            WHEN OTHER THEN
                v_error_message := ''Failed to process structured failure table: '' || SQLERRM;
                UPDATE DQ_RULE_AUDIT_LOG SET END_TIMESTAMP = CURRENT_TIMESTAMP(), STATUS = ''FAILED'', ERROR_MESSAGE = :v_error_message WHERE DATASET_RUN_ID = :v_run_id AND RULE_CONFIG_ID = :v_check_config_id AND STEP_NAME = :v_step;
                RETURN v_execution_error;
        END;
    ELSE
    ...
    BEGIN
        v_observed_value := v_expected_value; 
        LET details_json_str STRING := ''{"column": "'' || COALESCE(v_column_nm, ''null'') || ''", "expected_value": '' || v_expected_value || ''}'';
        ...
        v_sql := ''INSERT INTO "'' || v_dq_db_name || ''"."'' || v_dq_schema_name || ''".DQ_RULE_RESULTS (
                ...
                COALESCE(''null'', ''null'') || '', '''''' || COALESCE(''null'', ''null'') || '''''''';
...
EXCEPTION
    WHEN OTHER THEN
        ...
        VALUES (COALESCE(:v_run_id, -1), COALESCE(:v_check_config_id, -1), COALESCE(:v_procedure_name, ''SP_VALUE_LENGTH_EQUAL_CHECK''), COALESCE(:v_step, ''UNKNOWN''), CURRENT_TIMESTAMP(), CURRENT_TIMESTAMP(), ''FAILED'', :v_error_message);
