...
DECLARE
    ...
    -- Value Not In Set specific variables
    v_value_set_list ARRAY;
...
BEGIN
    ...
    v_procedure_name := COALESCE(RULE:PROCEDURE_NAME::STRING, ''SP_VALUE_NOT_IN_SET_CHECK'');
    ...
    BEGIN
        ...
        ELSEIF (UPPER(v_dataset_type) != ''TABLE'' AND UPPER(v_dataset_type) != ''QUERY'') THEN
            ...
            UPDATE DQ_RULE_AUDIT_LOG SET END_TIMESTAMP = CURRENT_TIMESTAMP(), STATUS = ''FAILED'', ERROR_MESSAGE = :v_error_message, LOG_MESSAGE = :v_input_rule_str WHERE DATASET_RUN_ID = :v_run_id AND RULE_CONFIG_ID = :v_check_config_id AND STEP_NAME = :v_step;
            RETURN v_status_code;
        ELSEIF (v_column_nm IS NULL OR v_value_set_list IS NULL OR ARRAY_SIZE(v_value_set_list) = 0) THEN
            v_error_message := ''COLUMN_NAME and non-empty VALUE_SET are required for SP_VALUE_NOT_IN_SET_CHECK.'';
            v_status_code := v_execution_error;
            UPDATE DQ_RULE_AUDIT_LOG SET END_TIMESTAMP = CURRENT_TIMESTAMP(), STATUS = ''FAILED'', ERROR_MESSAGE = :v_error_message, LOG_MESSAGE = :v_input_rule_str WHERE DATASET_RUN_ID = :v_run_id AND RULE_CONFIG_ID = :v_check_config_id AND STEP_NAME = :v_step;
            RETURN v_status_code;
        END IF;
    ...
    -- Build condition string from set
    SELECT ARRAY_TO_STRING(ARRAY_AGG('''''''' || REPLACE(f.value::STRING, '''''''', '''''''''''') || '''''''' ), '','') 
    INTO v_in_set_condition
    FROM TABLE(FLATTEN(input => :v_value_set_list)) AS f;
    ...
    -- For NOT_BE_IN_SET: unexpected records are those where the value IS in the set
    v_where_clause_condition := ''"'' || v_column_nm || ''" IS NOT NULL AND CAST("'' || v_column_nm || ''" AS VARCHAR) IN ('' || v_in_set_condition || '')'';
...
EXCEPTION
    WHEN OTHER THEN
        ...
        VALUES (COALESCE(:v_run_id, -1), COALESCE(:v_check_config_id, -1), COALESCE(:v_procedure_name, ''SP_VALUE_NOT_IN_SET_CHECK''), COALESCE(:v_step, ''UNKNOWN''), CURRENT_TIMESTAMP(), CURRENT_TIMESTAMP(), ''FAILED'', :v_error_message);
