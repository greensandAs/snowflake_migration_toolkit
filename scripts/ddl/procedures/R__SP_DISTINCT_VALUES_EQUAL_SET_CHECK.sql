...
CREATE OR REPLACE PROCEDURE "SP_DISTINCT_VALUES_EQUAL_SET_CHECK"("RULE" VARIANT)
RETURNS NUMBER(38,0)
LANGUAGE SQL
EXECUTE AS CALLER
AS '
DECLARE
    ...
    v_procedure_name STRING DEFAULT ''SP_DISTINCT_VALUES_EQUAL_SET_CHECK'';
...
---
    ...
    -- FIX: Use ARRAY_EXCEPT directly on the arrays. This is the simplest and correct way.
    v_missing_arr := (
    ...
    -- Success only if both arrays are empty (Exact Match)
    v_is_success := (ARRAY_SIZE(v_missing_arr) = 0 AND ARRAY_SIZE(v_extra_arr) = 0); 
...
EXCEPTION
    WHEN OTHER THEN
        BEGIN
            ...
            VALUES (COALESCE(:v_run_id, -1), COALESCE(:v_check_config_id, -1), COALESCE(:v_procedure_name, ''SP_DISTINCT_VALUES_EQUAL_SET_CHECK''), COALESCE(:v_step, ''UNKNOWN''), CURRENT_TIMESTAMP(), CURRENT_TIMESTAMP(), ''FAILED'', :v_error_message);
        END;
