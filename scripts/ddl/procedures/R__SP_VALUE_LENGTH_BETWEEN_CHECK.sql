...
CREATE OR REPLACE PROCEDURE "SP_VALUE_LENGTH_BETWEEN_CHECK"("RULE" VARIANT)
RETURNS NUMBER(38, 0)
LANGUAGE SQL
EXECUTE AS CALLER
AS '
DECLARE
    ...
    -- Length Check specific variables
    v_min_length NUMBER;
    v_max_length NUMBER;
    v_where_clause_condition STRING;
    
    -- Variables for keys and data source
...
BEGIN
    ...
    v_procedure_name := COALESCE(RULE:PROCEDURE_NAME::STRING, ''SP_VALUE_LENGTH_BETWEEN_CHECK'');
    ...
    BEGIN
        ...
        -- Length specific kwargs extraction
        v_min_length := v_kwargs_variant:min_value::NUMBER;
        v_max_length := v_kwargs_variant:max_value::NUMBER;
        ...
        -- Strict Parameter Validation from original script
    ...
    -- Swap logic here: Unexpected if Length is outside of [min, max]
    v_where_clause_condition := ''(LEN("'' || v_column_nm || ''") < '' || COALESCE(v_min_length::STRING, ''0'') || 
                                 '' OR LEN("'' || v_column_nm || ''") > '' || COALESCE(v_max_length::STRING, ''9999999'') || '')'';

    v_sql := ''SELECT COUNT(*) AS total_count, COUNT_IF('' || v_where_clause_condition || '') AS unexpected_count FROM '' || v_from_clause || '' WHERE 1=1 '' || v_incremental_filter;
    ...
    IF (v_error_message IS NULL) THEN
        BEGIN
            ...
            v_status_code := CASE WHEN (1 - v_percent) >= v_allowed_deviation THEN v_success_code ELSE v_failed_code END;
        EXCEPTION
            WHEN OTHER THEN
    ...
    IF (v_unexpected > 0 AND UPPER(v_dataset_type) = ''TABLE'') THEN
        BEGIN
            ...
            IF (v_key_column_names IS NOT NULL) THEN
                SELECT LISTAGG('''''''' || TRIM(value) || '''''''' || '', '' || TRIM(value), '', '') INTO v_key_parts_list FROM TABLE(SPLIT_TO_TABLE(:v_key_column_names, '',''));
                v_key_construct_expr := ''OBJECT_CONSTRUCT('' || v_key_parts_list || '')'';
                v_sql := ''INSERT INTO DQ_FAILED_ROW_KEYS (DATASET_RUN_ID, RULE_CONFIG_ID, DATABASE_NAME, SCHEMA_NAME, TABLE_NAME, FAILED_KEY) SELECT '' || v_run_id || '', '' || v_check_config_id || '', '''''' || COALESCE(v_database_name, ''N/A'') || '''''', '''''' || COALESCE(v_schema_name, ''N/A'') || '''''', '''''' || COALESCE(v_table_name, ''N/A'') || '''''', '' || v_key_construct_expr || '' FROM '' || v_from_clause || '' WHERE '' || v_where_clause_condition || v_incremental_filter;
    ...
    IF (v_unexpected > 0) THEN
        BEGIN
            ...
            v_sql := ''CREATE TABLE IF NOT EXISTS '' || v_full_target_table_name || '' AS SELECT '' || v_run_id || ''::NUMBER(38,0) AS DATASET_RUN_ID, '' || v_data_asset_id || ''::NUMBER(38,0) AS DATASET_ID, '' || v_check_config_id || ''::NUMBER(38,0) AS RULE_CONFIG_ID, CURRENT_TIMESTAMP()::TIMESTAMP_LTZ AS DQ_LOAD_TIMESTAMP, * FROM '' || v_from_clause || '' WHERE 1=0'';
            EXECUTE IMMEDIATE v_sql;

            v_sql := ''INSERT INTO '' || v_full_target_table_name || '' SELECT '' || v_run_id || '', '' || v_data_asset_id || '', '' || v_check_config_id || '', CURRENT_TIMESTAMP(), * FROM '' || v_from_clause || '' WHERE '' || v_where_clause_condition || v_incremental_filter || CASE WHEN v_failed_rows_cnt_limit > 0 THEN '' LIMIT '' || v_failed_rows_cnt_limit ELSE '''' END;
            EXECUTE IMMEDIATE v_sql;
            v_rows_inserted := SQLROWCOUNT;
    ...
    IF (v_error_message IS NULL) THEN
        BEGIN
            LET results_json_str STRING := ''{"element_count": '' || COALESCE(v_total::STRING, ''null'') || '', "unexpected_count": '' || COALESCE(v_unexpected::STRING, ''null'') || '', "unexpected_percent": '' || COALESCE(v_percent*100::STRING, ''null'') || '', "failed_records_table": "'' || COALESCE(v_failed_records_table, ''null'') || ''"}'';
            v_sql := ''INSERT INTO "'' || v_dq_db_name || ''"."'' || v_dq_schema_name || ''".DQ_RULE_RESULTS (BATCH_ID, DATASET_RUN_ID, DATASET_ID, RULE_CONFIG_ID, EXPECTATION_ID, RUN_NAME, RUN_TIMESTAMP, DATASET_NAME, EXPECATION_CONFIG, IS_SUCCESS, RESULTS, EXPECTATION_NAME, DETAILS, ELEMENT_COUNT, UNEXPECTED_COUNT, UNEXPECTED_PERCENT, UNEXPECTED_PERCENT_NONMISSING, UNEXPECTED_PERCENT_TOTAL, FAILED_ROWS) SELECT '' || COALESCE(v_batch_id::STRING, ''null'') || '', '' || COALESCE(v_run_id::STRING, ''null'') || '', '' || COALESCE(v_data_asset_id::STRING, ''null'') || '', '' || COALESCE(v_check_config_id::STRING, ''null'') || '', '' || COALESCE(v_expectation_id::STRING, ''null'') || '', '''''' || REPLACE(COALESCE(v_run_name, ''null''), '''''''', '''''''''''') || '''''', CURRENT_TIMESTAMP(), '''''' || REPLACE(COALESCE(v_data_asset_name, ''null''), '''''''', '''''''''''') || '''''', PARSE_JSON('''''' || REPLACE(COALESCE(RULE::STRING, ''null''), '''''''', '''''''''''') || ''''''), '' || CASE WHEN v_status_code = v_success_code THEN ''TRUE'' ELSE ''FALSE'' END || '', PARSE_JSON('''''' || REPLACE(results_json_str, '''''''', '''''''''''') || ''''''), '''''' || REPLACE(COALESCE(v_expectation_name, ''null''), '''''''', '''''''''''') || '''''', PARSE_JSON(''''{}''''), '' || COALESCE(v_total::STRING, ''null'') || '', '' || COALESCE(v_unexpected::STRING, ''null'') || '', '' || COALESCE(v_percent*100::STRING, ''null'') || '', NULL, NULL, NULL'';
            EXECUTE IMMEDIATE v_sql;
        EXCEPTION
            WHEN OTHER THEN
...
EXCEPTION
    WHEN OTHER THEN
        ...
        INSERT INTO DQ_RULE_AUDIT_LOG (DATASET_RUN_ID, RULE_CONFIG_ID, PROCEDURE_NAME, STEP_NAME, START_TIMESTAMP, END_TIMESTAMP, STATUS, ERROR_MESSAGE) VALUES (COALESCE(:v_run_id, -1), COALESCE(:v_check_config_id, -1), :v_procedure_name, COALESCE(:v_step, ''UNKNOWN''), CURRENT_TIMESTAMP(), CURRENT_TIMESTAMP(), ''FAILED'', :v_error_message);
