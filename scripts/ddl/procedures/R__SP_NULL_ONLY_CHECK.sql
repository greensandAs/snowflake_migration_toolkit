...
CREATE OR REPLACE PROCEDURE "SP_NULL_ONLY_CHECK"("RULE" VARIANT)
RETURNS NUMBER(38, 0)
LANGUAGE SQL
EXECUTE AS CALLER
AS '
...
BEGIN
    ...
    IF (v_unexpected > 0) THEN
        BEGIN
            ...
            v_sql := ''CREATE TABLE IF NOT EXISTS '' || v_failed_records_table || '' AS '' ||
                     ''SELECT '' ||
            ...
            v_sql := ''INSERT INTO '' || v_failed_records_table || '' '' ||
                     ''SELECT '' ||
                     v_run_id || '', '' ||
                     v_data_asset_id || '', '' ||
                     v_check_config_id || '', '' ||
