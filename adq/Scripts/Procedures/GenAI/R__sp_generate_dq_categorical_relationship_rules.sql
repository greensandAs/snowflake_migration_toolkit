...
-- DDL for PROCEDURE: DQ_FRAMEWORK.METADATA.GENERATE_DQ_CATEGORICAL_RELATIONSHIP_RULES(VARCHAR, VARCHAR, ARRAY)
-- Extracted on: 2025-10-27 10:25:01
--------------------------------------------------------------------------------
CREATE OR REPLACE PROCEDURE GENERATE_DQ_CATEGORICAL_RELATIONSHIP_RULES(
    "DATASET_NAME" VARCHAR, "DQ_CR_RULES_TABLE" VARCHAR, "FINAL_CAT_COL" ARRAY
)
RETURNS VARIANT
LANGUAGE PYTHON
...
def generate_dq_categorical_relationship_rules_handler(session: Session, dataset_name: str, dq_cr_rules_table: str, final_cat_col: list):
    try:
        ...
        if final_dq_rules:
            ...
            snowpark_df.write.mode("append").save_as_table(dq_cr_rules_table)
